...
def pre_process(latest_file: Path = None):
    ...
    # 만약에 '본번' 컬럼의 값이 358부터 391까지의 값이라면 '1구역'컬럼에 True를 넣어주고 아니라면 False를 넣어준다.
    df["1구역"] = df["본번"].between(358, 391)
    df["대지면적평당가격"] = df["거래금액"] / (df["대지면적"] / 3.306)
    df["대지면적평당가격"] = df["대지면적평당가격"].round(-1)
    df["대지면적평당가격"] = df["대지면적평당가격"].astype(int)
    # %%
    # df에서 번지, 본번, 부번, 건물명, 전용면적, 대지면적, 계약날짜, 거래금액, 건축년도, 대지면적평당가격, 양재2동, 1구역 컬럼만 가져온다.
    df = df.loc[
        :,
        [
            "번지",
